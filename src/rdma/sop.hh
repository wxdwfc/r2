...
public:
  ...
  inline auto execute(Arc<RC> &qp, R2_ASYNC) -> Result<ibv_wc> {
    return execute(qp, IBV_SEND_SIGNALED, R2_ASYNC_WAIT);
  }

  inline auto execute_sync(Arc<RC> &qp, int flags,
                           const double &timeout_usec = 1000000)
      -> Result<ibv_wc> {
    ibv_wc wc;

    {
      auto res = qp->send_normal(
          {.op = this->op,
           .flags = flags,
           .len = this->size,
           .wr_id = 0},
  ...
  inline auto execute(const Arc<RC> &qp, int flags, R2_ASYNC) -> Result<ibv_wc> {
    // the return
